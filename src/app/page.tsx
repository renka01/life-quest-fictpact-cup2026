...
export default function Home() {
  ...
  // Handler Ganti Menu dengan Transisi
  const handleMenuChange = (menuName: string) => {
    if (menuName === activeMenu) return;
    
    // Play Sound Effect
    const audio = new Audio('/sounds/glitch.mp3'); // Pastikan file ada di public/sounds/
    audio.volume = 0.5;
    audio.play().catch(() => {}); // Catch error jika user belum interaksi

    setIsTransitioning(true);
    setTimeout(() => { setActiveMenu(menuName); }, 400); // Ganti konten tepat saat shutter tertutup (400ms)
    setTimeout(() => { setIsTransitioning(false); }, 800); // Selesai animasi
  };
    ...
    <div className="min-h-screen bg-[#1a1b26] text-slate-200 font-mono flex overflow-hidden selection:bg-purple-500 selection:text-white relative">
      <style>{`
        ...
        @keyframes shutterTop {
          0% { height: 0; }
          40% { height: 50%; } /* Tertutup rapat */
          60% { height: 50%; } /* Tahan sebentar */
          100% { height: 0; }
        }
              ...
              {showCoinAnim && (
                <div className="absolute top-full mt-4 right-0 w-max bg-[#24283b] border-2 border-amber-500 px-4 py-2.5 rounded-none shadow-[4px_4px_0_#000] flex items-center gap-3 animate-in slide-in-from-top-2 fade-in duration-300 z-[60] pointer-events-none">
                  ...
                  <div className="flex flex-col text-left">
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider leading-none mb-0.5">Pendapatan</span>
